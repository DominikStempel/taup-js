...
import { integrateLayer, depthToRadius } from "./integration";
...
export interface RayResult {
  ...
  piercePoints: Array<{ depth: number; distanceDeg: number; time: number }>;
}
...
export function raySegment(
  ...
  for (const layer of model.layers) {
    if (layer.botDepth <= d1 + 1e-9 || layer.topDepth >= d2 - 1e-9) continue;

    const segTop = Math.max(layer.topDepth, d1);
    const segBot = Math.min(layer.botDepth, d2);
    if (segBot - segTop < 1e-9) continue;

    const topV = layer.velocityAt(segTop, isS);
    const botV = layer.velocityAt(segBot, isS);
    if (topV <= 0 || botV <= 0) continue;

    const { dT, dX } = integrateLayer(p, segTop, segBot, topV, botV, model.earthRadius);
    T += dT;
    X += dX;
  }
  return { T, X };
...
export function findTurningDepth(
  ...
  for (const layer of model.layers) {
    if (layer.botDepth <= minDepth + 1e-9 || layer.topDepth >= maxDepth - 1e-9) continue;
    if (isS && layer.topVs <= 0 && layer.botVs <= 0) continue;

    const segTop = Math.max(layer.topDepth, minDepth);
    const segBot = Math.min(layer.botDepth, maxDepth);

    const r1 = depthToRadius(segTop, R);
    const r2 = depthToRadius(segBot, R);
    ...
    if (u1 >= p && u2 <= p) {
      if (Math.abs(u1 - u2) < 1e-14) return segTop;
...
function bisectRayParam(
  ...
  let bracketLo: { p: number; x: number } | null = null;
  let bracketHi: { p: number; x: number } | null = null;
  ...
  if (!bracketLo || !bracketHi) return null;
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (xMid === null) {
      ...
      if (xNudge !== null) {
        if ((xNudge - targetX) * (loX - targetX) > 0) { loP = pNudge; loX = xNudge; }
        else                                            { hiP = pNudge; hiX = xNudge; }
      }
      continue;
    }
    if (Math.abs(xMid - targetX) < 1e-8) return pMid;
    if ((xMid - targetX) * (loX - targetX) > 0) { loP = pMid; loX = xMid; }
    else                                          { hiP = pMid; hiX = xMid; }
  }
  return (loP + hiP) / 2;
}
...
export function computeDirectRay(
  ...
  const p = bisectRayParam(
    pp => directRayDistance(model, pp, sourceDepth, maxSearchDepth, isS),
    pMin, pMax, targetX
  );
...
export function computeReflectedRay(
  ...
  const computeX = (p: number): number | null => {
    const r_ref = depthToRadius(reflectorDepth, R);
    const v_ref = model.velocityAt(reflectorDepth, isS);
    if (v_ref <= 0) return null;
    const vf_ref = v_ref * (R / r_ref);
    if (p >= 1 / vf_ref) return null;
...
export function computeSKS(
  ...
  const computeX = (p: number): number | null => {
    ...
    if (p >= pCMBs || p >= pICBp) return null;
  ...
  const pMax = Math.min(
    maxRayParam(model, cmbDepth, true),
    maxRayParam(model, cmbDepth, false)
  ) * 0.9999;
...
export function computeDiffractedRay(
  ...
  const r_cmb = depthToRadius(cmbDepth, R);
